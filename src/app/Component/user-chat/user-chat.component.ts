...
export class UserChatComponent implements OnInit, AfterViewInit {
  ...
  constructor(private cdr: ChangeDetectorRef, private connectuserservice: ConnectUserService, private chatService: ChatServicesService) { }
  ...
  sendMessage() {
    ...
    this.chatService.UserSendMSG(UserData).subscribe({
      next: (response) => {},
      error: () => {
